...
| # | 内容 |
|---|---|
| **G-2**<br>（最重要） | **ランキング記事を NFR-05 の分母から外すか**（未決事項 (c)）。選定側では区分⑦から除外し「対象内の月次統計だが値が取れない」行に絞ったが、**分母の定義そのもの**なので決め方によっては区分⑧の期待値が動く |
| **G-4** | **分母の帰属が未定義のマクロ統計行**（海外 GDP / 米 CPI / 消費者心理 / 消費支出 / 農水省統計）。`cpi` / `ec-platform` 以外のマクロ統計はカタログに segment が無く、`no_segment_match`（取りこぼし）と `out_of_scope`（対象外）のどちらが正しいかで NFR-05 の分母が変わる |
| G-1 | **`主要4業態 × 既存店` に一致する一意 URL は 12 件しかない**（G1 は 18 件を要求）。不足 6 件は「主要 4 業態の月次統計 かつ 値あり」で補った（**個社決算は混ぜていない**。初版は個社決算 3 件が混入していた） |
| G-5 | G1 の表記ゆれ 4 要素のうち **「全角数字」は母集団に 0 件**。延べ 595 行に 1 件も無く、区分④で構造的に充足できない。要件 FR-05 の記述との差として記録するか |

なお **G-3（`ホームセンター月次実績＝2026年6月度` を入れるか）は解消済み**。ランキング記事を
除外して「対象内の月次統計」に絞った結果、G1 が名指しするこの代表例が区分⑦に自然に入った。
