...
#### D-H. サイト公開を先に進める判断（2026-08-02 決定）

**② を採用した（オーナー判断）。** M4 → M6 を先に進めて実物を出し、SC-06 に
NFR-05 = 68.4% 未達を明示して公開する。設計 §8 は「NFR-04 と NFR-05 の双方を
満たすこと。いずれかが未達なら M3 に留まる」としており**形式的には M3 に留まる**
判定だが、残る 17 行は実装で直せる性質のものではないため、他部署の応答を待つ間に
配信側を進める。

C-2 / C-3 は cc-sier に起票済み:

| # | issue | 宛先 |
|---|---|---|
| C-3 | [#728](https://github.com/SAS-Sasao/cc-sier-organization/issues/728) 実装設計 §4.3 の内部不整合 3 件 | 技術リサーチ室 |
| C-2 | [#729](https://github.com/SAS-Sasao/cc-sier-organization/issues/729) カタログの `売上高` 曖昧性 | 小売ドメイン室 |

**#728 の起票時に、当初の想定より重い問題が確認できた。** C-3a は
`ドラッグストア／2月既存店売上ツルハ4.0%増、コスモス薬品7.0%増` のような行で
(1) ツルハ個社の値が `drugstore` 業態の観測になり（カタログ §1.4 違反）、
(2) コスモス薬品の値が observation にも unresolved にも現れず**痕跡なく消える**
（FR-10 違反）という 2 つの事故を同時に起こす。設計の衝突検出は observation が
1 件しか生成されないため発火しない。実データで 16 行、C-3b は 19 行。

#### ~~★ 次に決めてほしいこと: サイト公開を先に進めるか~~（D-H で決着）
