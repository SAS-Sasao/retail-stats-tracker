...
**① サイトの再生成分を cc-sier へ反映（1 往復で終わる）**

公開ページは **NFR-05 63.7% 時点**の内容のまま。現在の main で生成すると
67.1% に更新され、`company_disclosure` 16 件が SC-06 の P3 に第 3 区分として出る。

```bash
cd scripts/retail-stats-tracker
RETAIL_STATS_WORKSPACE=/path/to/cc-sier-organization   python3 -m retail_stats build --rebuild --no-llm
# → cc-sier の docs/retail-stats/index.html と data/*.json を PR
```

**配信ページは生成物である。** cc-sier 側で直接編集せず、必ず
`retail_stats/html/template.html` を直してから再生成すること。
